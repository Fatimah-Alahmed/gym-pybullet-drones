...
class BaseAviary_random(gym.Env):
    ...
    def reset(self,
        ...
        self.INIT_XYZS=np.random.uniform(low=-5, high=5, size=(self.NUM_DRONES,3 ))
        self.INIT_XYZS[:,2]=self.COLLISION_H/2-self.COLLISION_Z_OFFSET+.1
        
        p.resetSimulation(physicsClientId=self.CLIENT)
        #### Housekeeping ##########################################
        self._housekeeping()
        # Set a new random target pose
        self.TARGET_POS = self._generate_random_target_pose()
        #### Update and store the drones kinematic information #####
        self._updateAndStoreKinematicInformation()
        #### Start video recording #################################
        self._startVideoRecording()
